...
all_forest_exits = [(key, locations.get(key)) for key, value in exits.items() if 5 in value.values()]
print(all_forest_exits)

all_forest_exits = []
for key, value in exits.items():
    if 5 in value.values():
        all_forest_exits.append((key, locations.get(key)))

print(all_forest_exits)
print()

for k in locations:
    destination = [(key, locations.get(key)) for key, value in exits.items() if k in value.values()]
    print("Exit to location {}".format(k))
    for item in destination: print("\t{}".format(item))

for k in locations:
    destinations = []
    for key, value in exits.items():
        if k in value.values():
            destinations.append((key, locations.get(key)))
    print("Exit to location {}".format(k))
    for item in destinations: print("\t{}".format(item))
